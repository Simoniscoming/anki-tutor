...
评估思路：拿到复习记录后，用 `type`（**实测：0=学习 / 1=复习 / 2=relearn**）+ `ease`（**按钮：1=Again忘 / 2=Hard / 3=Good / 4=Easy；答对=ease≥2**）算保留率——保留率 = (ease≥2 的复习记录数) / (type==1 的复习记录数)。某个 deck 的失败率持续偏高 → 提示用户"这批卡可能质量低/太难，考虑拆细或降量"。
...
### 坑 2：中文牌组名 / 特殊字符（两套语法，别混用）

AnkiConnect 在涉及牌组名时走的是**两套完全不同的语法**，混用就会踩坑：

**路径 A — JSON 参数**（`addNote` 的 `deckName`、`createDeck`、`changeDeck` 等）
直接写原始牌组名即可。要点：
- 中文要确保 UTF-8 编码正确（Git Bash + curl `-d` 易出编码错，优先用 `--data-binary @文件` 传 UTF-8 JSON）
- 牌组名含 `::` 是正常的（Anki 层级分隔符），不是 bug
- **有些牌组返回 null/false 不是 bug**——它们没有独立配置组（继承父级），查它的父牌组

**路径 B — 搜索查询**（`findCards` / `findNotes` 的 `query`）
这里跑的是 Anki 的**搜索语法**，不是直接传牌组名：
- `deck:` 后接的牌组名若含空格，**必须加引号**：写 `"deck:git 杂项"` 或 `deck:"git 杂项"`。
  否则空格会把查询拆成 `deck:git` + `杂项` 两个条件，**静默返回 `[]` 且不报错**。
- 这是最危险的一种静默失败——拿到空结果会误判成"牌组是空的"，进而触发错误的数据操作（误删、漏查重）。

**防御（核心习惯）**：`findCards`/`findNotes` 拿到**空结果**时，在当结论前**先交叉验证**——
换一种查询写法（加引号 / 去掉 `deck:` 限定只搜关键词），或先 `deckNames` 核对牌组名拼写。
